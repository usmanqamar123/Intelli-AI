...
const IntelliHeader: React.FC<HeaderProps> = ({ aboutRef, pricingRef }) => {
  ...
  useEffect(() => {
    ...
    console.log("Extracted Token from URL:", extractedToken); // Debugging
  ...
  const logout = () => {
    ...
    router.push("/");
    storage.remove("intellitokens", function (error: any) {
      if (error) {
    ...
    storage.remove("intellinameLetter", function (error: any) {   
      if (error) {
        console.error("Error:", error);
      } else {
        console.log("Name letter removed successfully.");
      }
    }
    );
  };
  ...
  const storage = createGuest('https://app.intelliwriter.io/'); // ✅ host domain here
  // const storage = createGuest(`http://localhost:3001`); // ✅ host domain here

  useEffect(() => {
    storage.get("intellitokens", function (error: any, value: any) {
      if (error) {
        console.error("Error:", error);
      } else {
    ...
    storage.get("intellinameLetter", function (error: any, value: any) {
      if (error) {
        console.error("Error:", error)
      } else {
        setNameLetter(value)
        setSessionLetter(value)
      }
    });
  }, [ storage, setNameLetter ]);

  
  return (
    <>
      {/* NAVBAR FOR LARGE SCREENS */}
      <div className="fixed z-50 hidden lg:block top-0 w-full h-auto bg-[#0F062C] border-b border-[#888] shadow-xl">
