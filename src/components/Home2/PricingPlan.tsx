...
const PricingCard: React.FC<{
  ...
  const handleClick = async () => {
    if (!tokens && !subscribed) {
      if (card.id === 0) {
        router.push(`${process.env.NEXT_PUBLIC_APP}auth/login`);
  ...
  return (
    <div
      className={`text-center md:text-start bg-white/5 hover:bg-white/10 transition duration-500 md:max-w-[650px] sm:w-[75wh] w-[75wh] relative overflow-hidden border-[#FFFFFF14] text-white text-opacity-70 shadow-lg rounded-xl space-y-5 border ${
      ...
      <div className="w-full flex flex-col justify-between gap-5 relative 2xl:px-6 xl:px-5 lg:px-4 px-3">
        ...
        <div className="flex justify-center py-3 w-full">
          {card.id === 0 && currentPlan === "Basic Plan" ? (
            ...
            <Button
              title={card.id === 0 ? "Subscribe Now" : "Subscribe Now"}
              btnType="button"
              onClick={handleClick}
              className="btn"
            />
        ...
        <div className="py-6">
          <ul className="flex flex-col gap-3 justify-center items-center text-center lg:justify-start lg:items-start lg:text-start">
            {card.featuresName.map((featurename, index) => (
              <li
                key={index}
                className="flex xl:gap-2 gap-1 justify-center items-center md:justify-start md:items-start text-center md:text-start xl:text-base lg:text-sm text-xs font-light text-white w-full"
              >
                <div className="saturate-[3] text-gray-400 lg:text-base text-sm mt-[5px]">
                  {["No Keyword Research", "No Free Images"].some((text) =>
                    featurename.includes(text)
                  ) ? (
                    card.name === "Basic Pack" ? (
                      <FaTimes />
                    ) : (
                      <FaCheck />
                    )
...
const PricingPlan: React.FC<PricingPlanProps> = ({
  ...
  return (
    <div
      className="w-full flex items-center flex-col justify-center pb-10 mt-16"
      id="pricing"
      ref={pricingRef}
    >
      <div className="xl:px-10 md:px-2 px-4 w-full container">
        {!isPricingPlanPage && (
          <>
            <h2 className="lg:text-6xl text-5xl lg:mt-0 md:mt-6 mt-12 font-normal relative p-4">
        ...
        <div className=" w-full">
          <div className="w-full flex flex-col justify-center items-center">
            ...
            <div className="w-full lg:w-[90%] grid grid-cols-1 md:grid-cols-3 justify-center xl:gap-10 gap-7 xl:px-10 md:p-6">
